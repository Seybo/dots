...
- After Manager review passes, enter `manager_finalizing`. Load the repository-owned root `.autowork.yml`, select components from the Review's `starting_commit_sha..HEAD` changed paths using the longest configured directory prefix, and run every selected command through a separate non-login `bash -c` process from its component directory. Run all selected commands even when one fails; an explicit empty command list is skipped.
