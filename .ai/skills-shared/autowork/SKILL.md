...
description: >-
  ...
  visible manager, worker, and reviewer panes, committing each code-changing
  iteration, collecting reviews/debates, and running final checks. In Pi, invoke via
...
This is a **Pi-only command skill**. Do not auto-use it from general requests; wait for explicit `/autowork`. The process in `agent-reviewer` is a participant, not the orchestrator, and must not run this skill directly.

```text
/skill:autowork
/autowork
/autowork --retry
/autowork --super-review claude|codex
...
V1 is fully implemented: init/resume, step implementation prompts to `agent-worker`, status-JSON waits, commits, blind Pi audits, independent reviews, finding classification, bounded debates, final checks, one final whole-branch `/super-review`, super-fix adjudication, Pi final review, manager-context production-readiness review, and `final_summary.md`. Unresolved debates pause for human arbitration — the manager must not decide which agent is correct.

COPYABLE OUTPUT - HARD RULE: Never use Markdown tables, ASCII tables, aligned columns, or cell-based layouts in user-facing Autowork responses or generated human-readable review artifacts. Use numbered blocks, headings, or bullet lists with one labeled field per line. Apply this to findings, classifications, debates, progress summaries, final summaries, and manager gates. Keep long reasoning as normal paragraphs so it can be selected and copied in terminal UIs.

`--super-review` selects the software running in `agent-reviewer` and is persisted in `autowork-log/config.yml`:

- `codex` (default): `agent-reviewer` is Pi using Codex.
- `claude`: `agent-reviewer` is Claude. It handles the same review stages and invokes `/super-review --agent codex` for the final panel.

Changing the selected agent during a waiting phase requires `--retry` after all workers are stopped.
...
- `references/review-protocol.md` — reviewer rules (`/gtm-revit`-style, no test/lint runs), finding classification, debate procedure.
...
4. `agent-reviewer` independently reviews the complete step diff according to `/gtm-revit`-style rules.
5. Pi applies accepted fixes; `/autowork` commits every code-changing fix iteration.
6. `agent-reviewer` reviews again.
7. If the worker and reviewer disagree, facilitate bounded debate rounds; after the round limit, pause for user arbitration.
8. Continue until all steps are accepted.
9. Run final full checks.
10. Run one final whole-branch `/super-review --agent <selected-agent>`; Pi adjudicates/fixes findings with `/claude-super-fix` rules and room to disagree.
11. Rerun final checks after super-review fixes; `agent-reviewer` gives a normal scoped review of fix commits.
12. Ask `agent-worker` to review all changes for issues, gaps, and improvements (ignoring very minor issues).
13. Stop for agent-manager's manager-context production-readiness review.
14. If agent-manager finds issues: one structured findings file → `autowork manager-review-fix` → automated Pi fix, checks, scoped reviewer pass → fresh manager gate.
15. Mark complete only after agent-manager concludes the result is production-ready.
...
- before `agent-reviewer` reviews, the worktree must be clean and must remain clean after review/debate
- no-code debate/resolution iterations do not create empty commits; they live in `autowork-log/` only

Core invariant:

```text
Pi must never produce two code-changing commits in a row without `agent-reviewer` reviewing the last commit.
```
...
2. **Orchestration** — the Ruby helper drives panes, prompts, status waits, commits, and banners per `references/runtime.md`. Never bypass `Tmux#send_prompt`; on any wait or resume question, follow the timeout/resume semantics there. Use `--retry` only after the operator confirms workers are no longer running and wants the current waiting stage sent again.
