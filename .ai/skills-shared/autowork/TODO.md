...
- three-pane discovery by exact title: `agent-manager`, `agent-worker`, `agent-reviewer`
...
- send reviewer pass prompt and wait for status JSON
- ask Pi to classify machine-readable reviewer findings
- commit accepted fixes as `Step N fix M`
- send accepted-fix commits back to agent-reviewer for review
- facilitate bounded worker/reviewer debate for disputed/deferred findings, pausing for operator arbitration if unresolved after the limit
...
- send final-check fix commits to agent-reviewer for review
...
- final whole-branch super-review gate after final checks pass:
  - send `/super-review` to `agent-reviewer` once, scoped to `review_base_ref...HEAD`
  ...
  - send findings to `agent-worker` for `/claude-super-fix`-style adjudication with room to disagree: `accept`, `accept_with_alternative_fix`, `dispute`, `skip`, `already_fixed`, `out_of_scope`, `follow_up`, `needs_user`
  - commit accepted code changes as `Super-review fix N`
  - rerun final checks after super-review fixes
  - send super-review fix commits to agent-reviewer for a normal scoped review, not another full super-review
  - wait until the reviewer accepts the complete super-review/fix loop
  - send the exact final-review prompt to `agent-worker` after the reviewer's loop
  - save Pi's final review separately for the manager gate; do not combine its findings with the reviewer's
...
- final manager-context production-readiness review gate:
  ...
  - automatically route manager findings to Pi, commit `Manager review fix N`, rerun full final checks, send a scoped reviewer pass, and loop on reviewer findings
  - return to a fresh manager gate after the reviewer accepts; never auto-pass manager context after a fix
  - require agent-manager to decide whether the final result is production-ready if the user does not perform another review
...
- `/autowork` can facilitate bounded worker/reviewer debate, but it must not adjudicate unresolved disagreement. If both agents still disagree after the configured round limit, it pauses for operator arbitration.
