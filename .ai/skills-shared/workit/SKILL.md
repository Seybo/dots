...
1. **Parse command arguments:**
   ...
   - reject commands that combine `step <step_number>` and `non-stop`; these modes are mutually exclusive
...
6. **Verify/setup the development branch before planning or implementation:**
   - before planning or making any production code, docs, bin, config, schema, or spec changes in the code working directory, check the current branch with:
     ```bash
...
9. **After completing implementation:**
   ...
   - skip this final full-task review in `step_mode`; the invocation that completes the whole task owns it
