...
`CLAUDE.local.md` pointer in each gtm checkout (`/Volumes/dev/projects/shaka/gtm/<Nth>/`);
...
- The registered project id is `shaka_gtm` (see `~/.ai/skills-shared/components/projects.yml`);
  task folders live under `/Volumes/dev/_tasks/shaka_gtm/`. Branches carry an `sc-<digits>`
